 ...
 */
declare function NumberedProperty<Name extends string, Count extends number>(): `${Name}${IntRange<
    0,
    Count
>}`;
/// Generic helper to add { name0: { ... } }
type NumberedComponents<Component, Name extends string, Count extends number> = {
    [k in ReturnType<typeof NumberedProperty<Name, Count>>]: Component;
};
...
type WithScreenButtons<Count extends number> = NumberedComponents<
    ScreenButton,
    'ScreenButton',
    Count
>;
...
type WithFlashMemorys<Count extends number> = NumberedComponents<
    FlashMemory<any>,
    'FlashMemory',
    Count
>;
type WithTypedFlashMemorys<Count extends number, T> = NumberedComponents<
    FlashMemory<T>,
    'FlashMemory',
    Count
>;
type WithGamepadChips<Count extends number> = NumberedComponents<GamepadChip, 'GamepadChip', Count>;
type WithKeyboardChips<Count extends number> = NumberedComponents<
    KeyboardChip,
    'KeyboardChip',
    Count
>;
type WithMagneticConnectors<Count extends number> = NumberedComponents<
    MagneticConnector,
    'MagneticConnector',
    Count
>;
...
type WithSecurityChips<Count extends number> = NumberedComponents<
    SecurityChip,
    'SecurityChip',
    Count
>;
...
type WithLedStrips<Count extends number, Size extends number> = NumberedComponents<
    LedStrip<Size>,
    'LedStrip',
    Count
>;
type WithSegmentDisplays<Count extends number, Size extends number> = NumberedComponents<
    SegmentDisplay<Size>,
    'SegmentDisplay',
    Count
>;
...
type WithSegmentDisplay<Size extends number> = NumberedComponents<
    SegmentDisplay<Size>,
    'SegmentDisplay',
    1
>;
